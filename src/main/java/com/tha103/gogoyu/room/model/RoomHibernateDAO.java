...
//			map.put((Room) rooms.get(0)[0], list);
	...
	public List<Object> getRoomProdutDetail(Integer roomId) {
		try {
			...
			Room room = getSession().get(Room.class, roomId);
			List<Integer> roomPhotoIdList = getSession()
					.createQuery("select room_photo_id from Room_photo where room_id =:room_id", Integer.class)
...
//			@SuppressWarnings("unchecked")
			NativeQuery<Room> query1 = getSession().createNativeQuery("SELECT * FROM room WHERE room_id IN "
					+ "(SELECT r.room_id FROM room r"
					+ " JOIN room_stock rs ON r.room_id = rs.room_id JOIN company c ON r.comp_id = c.comp_id WHERE (c.comp_address LIKE :comp_address) AND (rs.stock_date BETWEEN :checkIn AND :checkOut) AND (r.room_type <= :number) AND (rs.stock > 0) GROUP BY room_id)",
					Room.class).setParameter("comp_address", "%" + comp_address + "%").setParameter("checkIn", checkIn)
					.setParameter("checkOut", checkOut).setParameter("number", number);
			List<Room> list = query1.list();
			for (Room room : list) {
